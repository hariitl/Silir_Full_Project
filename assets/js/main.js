...
// Function to update temperature value from server
...
//async function updateTemperature1() { 
 // try {
   // const response = await fetch('/temperature');
   // const data = await response.json();
   // document.getElementById('temperature-value1').textContent = `${data.temperature.toFixed(1)}°C`;
 // }// catch (error) {
   // console.error("Error fetching temperature:", error);
 // }
//}
...
// Update temperature every 2 seconds
setInterval(updateTemperature1, 2000);
...
async function updatehumidity() {
  try {
   ...
   // document.getElementById('humidity-value1').textContent = `${data.humidity.toFixed(1)}%`;
  } catch (error) {
    console.error("Error fetching temperature:", error);
  }
}
...
async function updateCO2() {
  try {
    ...
    const data = await response.json();
    document.getElementById('co2-value').textContent = `${data.co2.toFixed(1)}ppm`;
  } catch (error) {
    console.error("Error fetching CO₂:", error);
  }
}

// Update CO₂ every 2 seconds
setInterval(updateCO2, 2000);

async function updateammonia() {
  try {
    const response = await fetch('/ammonia');
    const data = await response.json();
    document.getElementById('ammonia-value').textContent = `${data.ammonia.toFixed(1)}%`;
    document.getElementById('ammonia-value1').textContent = `${data.ammonia.toFixed(1)}%`;
  } catch (error) {
    console.error("Error fetching ammonia:", error);
  }
}

// Update moisture every 2 seconds
setInterval(updateammonia, 2000);
...
async function updateOther() {
  try {
    ...
    const data = await response.json();
    document.getElementById('other-value').textContent = `${data.other.toFixed(1)}%`;
  } catch (error) {
